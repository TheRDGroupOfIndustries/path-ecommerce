...
export const buyNow = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (referralCode) {
      ...
      if (
        !isNaN(extractedPercent) &&
        extractedPercent > 0 &&
        extractedPercent <= 100
      ) {
        referralDiscountPercent = extractedPercent;
        referralDiscountAmount =
          (priceAfterProductDiscount * referralDiscountPercent) / 100;
        priceAfterReferralDiscount =
          priceAfterProductDiscount - referralDiscountAmount;
        ...
        if (
          referralCode &&
          referralDetails &&
          !referralDetails.usedBy.includes(userId)
        ) {
        ...
        return res
          .status(400)
          .json({ message: "Invalid referral code format" });
      }
    }
...
//       return res
//         .status(404)
//         .json({ message: "Product associated with this order not found" });
//     }
...
//             (priceAfterDiscount * referralPercent) / 100;
//           finalPrice = priceAfterDiscount - referralDiscountAmount;
...
//           priceAfterProductDiscount: priceAfterDiscount,
//           referralDiscountPercent: referralCode
