...
// // const renderNestedLevels = (node, depth = 1, onEdit) => {
// //   if (!node) return [];
...
// //       <tr key={`${depth}-${index}`}>
// //         <td className="pl-4 py-2">{item.associates[0].level}</td>
// //         <td className="py-2 lwuser-commission-amount flex items-center gap-2">
// //           {formatCurrency(item.associates[0].totalCommissionInPercent)}%
// //           <button
// //             className="edit-btn"
// //             title="Edit"
// //             onClick={() => onEdit(item.associates[0])}
// //           >
// //             <Pencil size={14} />
// //           </button>
// //         </td>
...
// //       ? renderNestedLevels(item.lowerLevels, depth + 1, onEdit)
// //       : [];
...
//   return node.flatMap((item, index) => {
//     const user = item.associates[0];
//     const isEditing = editingUserId === user.id;

//     const currentRow = (
//       <tr key={`${depth}-${index}`}>
//         <td className="pl-4 py-2">{user.level}</td>
//         <td className="py-2 lwuser-commission-amount flex items-center gap-2">
//           {isEditing ? (
//             <>
//               <input
...
//               />
//               <button onClick={() => handleCommissionUpdate(user.id)}>
//                 <Check size={14} />
//               </button>
...
//                 title="Edit"
//                 onClick={() =>
//                   startEditing(user.id, user.totalCommissionInPercent)
//                 }
//               >
//                 <Pencil size={14} />
//               </button>
...
function LevelRow({
  ...
  const saveEdit = async (user) => {
    ...
    try {
      const level = user.level;
      const newPercent = Number.parseFloat(editedCommission);
      const response = await patchData("/tree/edit", { level, newPercent });
      if (response.success) {
        context.setAlertBox({ open: true, msg: response.message, error: false });
        setEditingRowId(null);
        onCommissionUpdate && onCommissionUpdate(user.id, newPercent);
      } else {
        alert("Failed to update commission: " + response.message);
      }
    } catch (error) {
